...
Partial Class _Receta
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            Session("menu") = "C"
            wucProductosActivos.ddlAutoPostBack = True
        End If
        Lmsg.Text = "" : gvPos = 0
        If Request.Form("btnSi") <> "" Then
            Dim ep As New ctiCatalogos
            Dim err As String = ep.eliminarReceta(CInt(Session("idz_e").ToString))
            GridView1.DataSource = ep.gvReceta(wucProductosActivos.idProducto)
            ep = Nothing
            GridView1.DataBind()
    ...
    Protected Sub GridView1_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GridView1.RowCommand
        ...
        ElseIf e.CommandName = "Editar" Then
            ...
            Dim datos() As String = dsP.datosReceta(CInt(GridView1.Rows(Convert.ToInt32(e.CommandArgument)).Cells(0).Text))
            dsP = Nothing
            If datos(0).StartsWith("Error") Then
                Lmsg.CssClass = "error"
                Lmsg.Text = datos(0)
            Else
                wucInsumos.idInsumo = datos(0)
                TxtCantidad.Text = datos(1)
                TxtIdReceta.Text = datos(2)
    ...
    Protected Sub btnActualizar_Click(sender As Object, e As EventArgs) Handles btnActualizar.Click
        ...
        Else
            ...
            Dim r As String = ap.actualizarReceta(idA, wucInsumos.idInsumo, TxtCantidad.Text, TxtIdReceta.Text)
            GridView1.DataSource = ap.gvReceta(wucProductosActivos.idProducto)
            ap = Nothing
            GridView1.DataBind()
    ...
    End Sub
    Protected Sub wucProductosActivosSeleccionado(sender As Object, e As System.EventArgs) Handles wucProductosActivos.productoSeleccionado
        Dim dsP As New ctiCatalogos
        Dim datos() As String = dsP.datosRecetaID(wucProductosActivos.idProducto)
        dsP = Nothing
        If datos(0).StartsWith("Error") Then
            Lmsg.CssClass = "error"
            Lmsg.Text = datos(0)
        Else
            TxtIdReceta.Text = datos(0)
        End If

        Dim gvds As New ctiCatalogos
        GridView1.DataSource = gvds.gvReceta(wucProductosActivos.idProducto)
        gvds = Nothing
        GridView1.DataBind()
        If IsNumeric(grdSR.Text) Then
            grdSR.Text = ""
            btnActualizar.CssClass = "btn btn-info btn-block btn-flat" : btnActualizar.Enabled = False
            TxtCantidad.Text = "" : wucInsumos.idInsumo = 0
    ...
    Protected Sub btnAgregar_Click(sender As Object, e As EventArgs) Handles btnAgregar.Click
        ...
        Else
            ...
            Dim r() As String = gp.agregarReceta(wucInsumos.idInsumo, TxtCantidad.Text, TxtIdReceta.Text)
            GridView1.DataSource = gp.gvReceta(wucProductosActivos.idProducto)
            gp = Nothing
            GridView1.DataBind()
